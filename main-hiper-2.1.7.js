...
function clickButtonSelect(button, index) {
    ...
    const select = document.querySelector(`.product-customizations div.custom-select select[name="${target}"]`);
    const options = document.querySelectorAll(`.selectbtn.target-${target}`);
    ...
    const selectIndex = Array.from(document.querySelectorAll('.product-customizations div.custom-select select')).indexOf(select);
    document.querySelectorAll('.product-customizations div.custom-select select').forEach((select, i) => {
        if (i > selectIndex) {
            waitElementToReplace(select, i);
        }
    });
}
...
function ReplaceSelectWithButtons(selectField, index) {
    ...
    document.querySelectorAll(`.product-customizations div.custom-select div[data-target="${selectId}"]`).forEach(select => {
        select.remove();
    });
