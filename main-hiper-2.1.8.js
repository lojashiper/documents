...
const createBuyButton = (element, parentElement, link_button) => {
    if (element) {
        if(!parentElement.querySelector('.buy-button-custom')){
            element.classList.add('hidden-button');
            element.insertAdjacentHTML('afterend', '<a href="'+ link_button +'" class="loader-button btn btn-primary buy-button-custom -clean">Comprar agora <svg viewBox="0 0 36 8"><circle cx="4" cy="4" r="4"><animate attributeName="opacity" dur="1s" values="0;1;0" repeatCount="indefinite" begin=".1"></animate></circle> <circle cx="18" cy="4" r="4"><animate attributeName="opacity" dur="1s" values="0;1;0" repeatCount="indefinite" begin=".2"></animate></circle> <circle cx="32" cy="4" r="4"><animate attributeName="opacity" dur="1s" values="0;1;0" repeatCount="indefinite" begin=".3"></animate></circle></svg></a>');
        }
      }
}

const createBuyFloatingButton = (element_floating, parentElement, link_button) => {
    if (element_floating) {
        if(!parentElement.querySelector('.buy-floating-button-custom')){
            element_floating.classList.add('hidden-button');
            element_floating.insertAdjacentHTML('afterend', '<a href="'+ link_button +'" class="loader-button btn btn-primary buy-floating-button-custom -clean" style="padding: unset"> Comprar <svg viewBox="0 0 36 8"><circle cx="4" cy="4" r="4"><animate attributeName="opacity" dur="1s" values="0;1;0" repeatCount="indefinite" begin=".1"></animate></circle> <circle cx="18" cy="4" r="4"><animate attributeName="opacity" dur="1s" values="0;1;0" repeatCount="indefinite" begin=".2"></animate></circle> <circle cx="32" cy="4" r="4"><animate attributeName="opacity" dur="1s" values="0;1;0" repeatCount="indefinite" begin=".3"></animate></circle></svg></a>');
        }
      }
}

const addBuyButton = async (link_button) => {
  const parentElement = document.querySelector('.main-product-buy-button-holder');
  const element = parentElement.querySelector('.main-product-buy-button-holder > button.btn-primary');
  const element_floating = parentElement.querySelector('.main-product-buy-button-holder .floating-button-container button.btn-primary');

  createBuyButton(element, parentElement, link_button);
  createBuyFloatingButton(element_floating, parentElement, link_button);
  
  return new Promise((resolve) => {
    const observer = new MutationObserver((mutationsList, observer) => {
      // Pare de observar mutações temporariamente
      observer.disconnect();

      createBuyButton(element, parentElement, link_button);
      createBuyFloatingButton(element_floating, parentElement, link_button);    

      // Comece a observar novamente
...
const createDestiny = (destiny_option, sku_product) => {
    if(destiny_option == 'ADO'){
      ...
      .then((data) => {
        ...
        fetch("http://api.abmexpaycheckout.com/checkout/lojas-hiper-checkout-abmex.myshopify.com", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
