...
function loading_on_product_page() {
    ...
    checkElement('.main-product-reference').then((selector) => {
        document.querySelector('.reference-availability').insertAdjacentHTML('afterend', '<span class="main-products-sold">Novo | ' + parseInt(document.querySelector('.product .actual-price').innerText.split(' ')[1].replace(',', '.')) * 77 + ' Vendidos</span>');
    ...
    checkElement('.main-product-info .main-product-buy-button-holder').then((selector) => {
        ...
        const variations_length = window.data['product']['data']['variations']['data']['length'];
